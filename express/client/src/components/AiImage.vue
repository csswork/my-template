<template>
              ...
              <div class="tools">
                <div class="form-item">
                  ...
                  <div class="input-wrap">
                    ...
                    <div class="upload-image" v-if="upload_image && upload_image_is_done">
                      <el-image :src="upload_image" fit="cover" alt="upload" />
  ...
  >
    ...
    <el-radio-group v-model="selected_type">
      <el-radio :value="type.value" border v-for="type in input_types" :key="type.value">{{ type.name }}</el-radio>
      
    </el-radio-group>
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" @click="is_show_image_dialog = false; upload_image_is_done = true;" size="large">
          保存
        </el-button>
      </div>
    </template>
...
<script setup>
  ...
  const is_show_image_dialog = ref(false);
  const images = ref([]);
  ...
  const upload_image_is_done = ref(false);
  ...
  const generateImage = () => {
    ...
    if (upload_image.value && upload_image_is_done.value) {
      // use image only for v2.0
      // data.value.req_key = 'high_aes_scheduler_svr_controlnet_v2.0';
      // data.value.model_version = 'general_controlnet_v2.0';
      data.value.req_key = 'high_aes_general_v21_L';
      data.value.model_version = 'general_v2.1_L';
      data.value.req_schedule_conf = 'general_v20_9B_pe';

      data.value.use_rephraser = true;


      data.value.controlnet_args = [];
      data.value.controlnet_args.push({
        binary_data_index: 0,
  ...
  const changeImage = async () => {
    ...
    try {
      ...
      is_show_image_dialog.value = true;
    } catch (error) {
  ...
  const resetImage = () => {
    upload_image.value = '';
    upload_image_is_done.value = false;
    
    if (data.value.binary_data_base64) delete data.value.binary_data_base64;
    if (data.value.controlnet_args) delete data.value.controlnet_args;
